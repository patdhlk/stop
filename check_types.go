...
func (tc *typeCheckArithmetic) TypeCheck(v *TypeCheck) (ast.Node, error) {
	...
	if mathType == ast.TFloat && tc.n.Op == ast.ArithmeticOpMod {
...
func (tc *typeCheckCall) TypeCheck(v *TypeCheck) (ast.Node, error) {
	...
	if function.Variadic && function.VariadicType != ast.TAny {
...
func (tc *typeCheckOutput) TypeCheck(v *TypeCheck) (ast.Node, error) {
	...
	if len(types) == 1 && types[0] == ast.TList {
		v.StackPush(ast.TList)
		return n, nil
	}

	// If there is only one argument and it is a map, we evaluate to a map
	if len(types) == 1 && types[0] == ast.TMap {
		v.StackPush(ast.TMap)
		return n, nil
	}
	...
	for i, t := range types {
		if t != ast.TString {
			cn := v.ImplicitConversion(t, ast.TString, n.Exprs[i])
			if cn != nil {
				n.Exprs[i] = cn
				continue
			}
			...
			return nil, fmt.Errorf(
				"output of an STOP expression must be a string, or a single list (argument %d is %s)", i+1, t)
		}
	}

	// This always results in type string
	v.StackPush(ast.TString)
