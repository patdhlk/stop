...
func TestInterpolationWalker_detect(t *testing.T) {
	cases := []struct {
...

		{
			...
			Result: []string{
				"Call(file, Literal(TString, test.txt))",
			},
		},

		{
			...
			Result: []string{
				"Call(file, Literal(TString, foo/bar.txt))",
			},
		},

		{
			...
			Result: []string{
				"Call(join, Literal(TString, ,), Variable(foo.bar.*.id))",
			},
		},

		{
			...
			Result: []string{
				"Call(concat, Literal(TString, localhost), Literal(TString, :8080))",
			},
		},
	}
